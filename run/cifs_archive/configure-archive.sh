...
function configure_archive () {
  ...
  then
    echo "//$archiveserver/$sharename $archive_path cifs credentials=${credentials_file_path},iocharset=utf8,file_mode=0777,dir_mode=0777,sec=ntlm 0" >> /etc/fstab
  fi
  ...
  then
    ...
    then
      echo "//$archiveserver/$musicsharename $music_archive_path cifs credentials=${credentials_file_path},iocharset=utf8,file_mode=0777,dir_mode=0777,sec=ntlm 0" >> /etc/fstab
    fi
  fi
